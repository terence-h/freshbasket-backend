    ...
    : BackgroundService
{
            ...
            try
            {
                ...
                foreach (var orderMessage in messages)
                {
                    try
                    {
                        ...
                        logger.LogInformation("Successfully processed order {OrderId}", orderMessage.OrderId);
                    }
                    catch (Exception ex)
                    {
    ...
    private async Task ProcessOrderAsync(Models.OrderMessage orderMessage, IOrderService orderService, ISqsService sqsService)
    {
